...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var calcPrice = {};
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])('.button-design', '.popup-design');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])('.button-consultation', '.popup-consultation');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])('.fixed-gift', '.popup-gift', true);
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.main-slider-item', 'vertical', 3000);
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.feedback-slider-item', 'horizontal', 3000, '.main-prev-btn', '.main-next-btn');
  Object(_modules_mask__WEBPACK_IMPORTED_MODULE_3__["default"])('[name="phone"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="name"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="message"]');
  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles');
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_6__["default"])('#size', '#material', '#options', '#promocode', '.calc-price', calcPrice);
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"])('form', calcPrice);
...
function accordion(triggersSelector) {
  var buttons = document.querySelectorAll(triggersSelector);
  buttons.forEach(function (button) {
    button.addEventListener('click', function () {
      // Для подсветки выбранного загодовка у нас есть класс active-style
      this.classList.toggle('active-style'); // В первом варианте использовался класс active на заголовках. Теперь, он будет использоватьна на контенте. Только переименуем его в active-content
      // Обратимся к следующему элементу после заголовка(контенту)

      this.nextElementSibling.classList.toggle('active-content'); // Мы должны узнать: нам нужно показать контент при клике на заголовок или скрыть?
...
function burger(menuSelector, burgerSelector) {
  ...
  var burgerElement = document.querySelector(burgerSelector); // Вручную скрываем меню

  menuElement.style.display = 'none'; // Отслеживание действий с бургером
...
function calc(size, material, options, promocode, result, state) {
  ...
  function getPriceFromServer(data, block) {
    data.forEach(function (item) {
      Object.values(item).forEach(function (element, index) {
  ...
  sizeBlock.addEventListener('change', function () {
    Object(_services_services__WEBPACK_IMPORTED_MODULE_2__["getResource"])('http://localhost:3000/sizeArt').then(function (data) {
      return getPriceFromServer(data, sizeBlock);
    });
    calculatedTotal();
...
function drop() {
  var fileInputs = document.querySelectorAll('[name="upload"]'); // Массив событий связаный с drag & drop

  ['dragenter', 'dragleave', 'dragover', 'drop'].forEach(function (eventName) {
    fileInputs.forEach(function (intup) {
  ...
  ['dragenter', 'dragover'].forEach(function (eventName) {
    fileInputs.forEach(function (intup) {
      intup.addEventListener(eventName, function () {
        return highlight(intup);
      }, false);
    });
  ...
  fileInputs.forEach(function (input) {
    ...
    input.addEventListener('drop', function (evt) {
      ...
      var dots; // item.files - это файл, который загрузили в input
      ...
      var arr = input.files[0].name.split('.'); // Проверяем длинну имени фото

      arr[0].length > 6 ? dots = '...' : dots = '.';
      var name = arr[0].slice(0, 6) + dots + arr[1]; // Элемент, куда нужно вывести название файла - находится перед инпутом
...
function filter() {
  ...
  var tabParent = document.querySelector('.portfolio-menu'); // Находим табы

  var tabItems = tabParent.querySelectorAll('li'); // Находим родитель фотографий

  var contentWrapper = document.querySelector('.portfolio-wrapper'); // Находим сами фото

  var contentItems = contentWrapper.querySelectorAll('div'); // // Находим блок с контентом вместо фото

  var no = document.querySelector('.portfolio-no'); // Собираем в массив классы фильтрации (у кнопок и контента класс один)

  var classesArr = ['.all', '.lovers', '.chef', '.girl', '.guy', '.grandmother', '.granddad']; // Функция для показа нужного контента
  ...
  function typeFilter(markType) {
    ...
    no.classList.remove('animated', 'fadeIn'); // Фильтрация
  ...
  tabParent.addEventListener('click', function (evt) {
    var target = evt.target;

    if (target && target.tagName == 'LI') {
      tabItems.forEach(function (item) {
        // Убираем активный класс у таба
        item.classList.remove('active'); // Назначаем активный класс кликнутому табу
  ...
  function getTabContent(className) {
    var trigger = tabParent.querySelector(className);
    var content = contentWrapper.querySelectorAll(className);
    trigger.addEventListener('click', function () {
      if (trigger.classList.contains('grandmother') || trigger.classList.contains('granddad')) {
        typeFilter();
      } else {
        typeFilter(content);
      }
  ...
  classesArr.forEach(function (el) {
    getTabContent(el);
  });
}
...
function forms(formSelector, state) {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var arr = item.files[0].name.split('.'); // Проверяем длинну имени фото
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (evt) {
      ...
      statusMessage.classList.add('status'); // Добавляем оповещение в родителя формы

      form.parentNode.append(statusMessage); // Скрываем форму, что бы показать статус отправка ^
      // Скрываем при помощи классов из animated.css и убираем со страницы

      form.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        form.style.display = 'none';
      }, 400); // Создаем элемент, в котором будет находится фото со статусом отправки

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.append(statusImg); // Текстовое оповещение отправки
      ...
      var api; // Ищет блок, который у данного элемента выше по иерархии (грубо говоря родителя, в котором находится форма), или если это форма, у которой класс calc_form

      form.closest('.popup-design') || form.classList.contains('calc_form') ? api = path.designer : api = path.question;
      console.log(api);
      Object(_services_services__WEBPACK_IMPORTED_MODULE_8__["postData"])(api, formData).then(function (data) {
        console.log(data);
        ...
        setTimeout(function () {
          statusMessage.remove(); // Возвращаем форму обратно
  ...
  function closeFormParent(parentFormSelector) {
    var modals = document.querySelectorAll(parentFormSelector);
...
function mask(selector) {
  ...
  var setCursorPosition = function setCursorPosition(pos, elem) {
    elem.focus(); // Устанавливаем выделение в input
  ...
  function createMask(event) {
    ...
    var matrix = '+7 (___) ___ __ __'; // Переменная итератор

    var i = 0; // Два значения
    // Статичное значение. Работает на основе матрицы. Получаем +7 при нажатии на input.

    var def = matrix.replace(/\D/g, ''); // Динамичное значение. Работает на основе того, что ввел пользователь. Позволяет ему вводить только цифры

    var val = this.value.replace(/\D/g, ''); // Если кол-во цифр, которые у нас остались в матрице после манипуляций def, больше или равно кол-ву цифр, которые в val, то это значение мы должны заменить на стандартное (то есть, пользователь не может удалить +7)
...
function modals(triggerSelector, modalSelector) {
  ...
  var modal = document.querySelector(modalSelector); // Собираем в коллекцию все модальные окна при помощи data-атрибута
  ...
  modalTrigger.forEach(function (btn) {
    btn.addEventListener('click', function (evt) {
      ...
      popups.forEach(function (popup) {
        popup.style.display = 'none'; // Анимация при помощи библиотеки animation.css
  ...
  function showModalByScroll() {
    var giftImg = document.querySelector('.fixed-gift');

    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1 && giftImg) {
...
function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    // Ищем фото в блоке
    var img = block.querySelector('img'); // Меняем атрибут src у img
    // image.png => image-1.png

    img.src = img.src.slice(0, -4) + '-1.png'; // Ищем в блоке все параграфы и скрываем их (все, кроме <p></p> с классом sizes-hit)
...
function scrolling(upSelector) {
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (evt) {
      evt.preventDefault(); // Какое расстояние было пролистано сверху

      var widthTop = document.documentElement.scrollTop;
      var hash = this.hash; // Верхняя граница элемента, к которому будем скроллить
      ...
      function step(time) {
        ...
        var progress = time - start; // Кол-во пикселей, которые нам необходимо пролистать в течении этой анимации
...
function showMoreStyles(buttonSelector) {
  ...
  /*#__PURE__*/
  function () {
    function styleCard(src, title, link, parentSelector) {
    ...
    _createClass(styleCard, [{
      key: "render",
      value: function render() {
        var card = document.createElement('div');
        this.classes.forEach(function (className) {
          return card.classList.add(className);
        });
        card.innerHTML = "\n              <div class=styles-block>\n              <img src=".concat(this.src, " alt>\n              <h4>").concat(this.title, "</h4>\n              <a href=").concat(this.link, ">\u041F\u043E\u0434\u0440\u043E\u0431\u043D\u0435\u0435</a>\n            </div>\n                ");
  ...
  button.addEventListener('click', function () {
    Object(_services_services__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles').then(function (data) {
      data.forEach(function (_ref) {
        var src = _ref.src,
            title = _ref.title,
            link = _ref.link;
        new styleCard(src, title, link, '.styles .row', 'col-sm-3', 'col-sm-offset-0', 'col-xs-10', 'col-xs-offset-1', 'animated', 'fadeInUp').render();
      });
...
function sliders(slides, dir, interval, prev, next) {
  ...
  var slideCurrentIndex = 1; // Переменная с автоматическим интервалом переключения слайдов
  ...
  try {
    ...
    prevSlideBtn.addEventListener('click', function () {
      plusSlideIndex(-1); // Добавляем классы при переключении слайдера из библиотека animations.css
  ...
  } catch (e) {} // Функция для автоматикечкого переключения слайдов
  ...
  activateAnimation(); // Обрашаемся к родителю слайдов
